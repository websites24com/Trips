...
exports.resizeUserPhoto = (req, res, next) => {
  ...
  req.file.filename = `user-${req.user.id}-${Date.now()}.jepg`;
...
exports.updateMe = catchAsync(async (req, res, next) => {
  ...
  // we must specified fields to permit user update only allowed ones
...
exports.deleteMe = catchAsync(async (req, res, next) => {
  ...
  res.status(204).json({
    status: 'success',
    message: 'Your account is not active now',
    data: null,
  });
});
